...
class PackageCtrl @Inject() (val silhouette: Silhouette[MyEnv[Users]], val messagesApi: MessagesApi, serviceAPI: ServicesAPI, fareRuleService: HotelFareRuleService, hotelCRUD: HotelCRUD, implicit val encrypt: Encrypt, implicit val mailService: MailService) extends WebController {
  ...
  def vacationPackages(page: Int, apply_filter: Boolean, country_id: Long, theme_id: String, pick_up: String, num_of_adult: Int, num_of_children: Int) = UserAwareAction.async { implicit request =>
    Future {
      val pageLength = 15
      if (apply_filter) {
        ...
        Ok(views.html.web.packages.result(expression.findList.toVector, Packages.find.where().eq("country_id", country_id).findRowCount(), page, pageLength, apply_filter, country_id, theme_id))
      } else {
        val offset = (page - 1) * pageLength
        val expression = Packages.find.where().raw(s" deleted = 0 order by id desc limit $pageLength offset $offset ").setIncludeSoftDeletes()
        Ok(views.html.web.packages.result(expression.findList.toVector, Packages.find.findRowCount(), page, pageLength, apply_filter, country_id, theme_id))
      }
    }
  }

  def select(id: String) = UserAwareAction { implicit request =>
    Ok(views.html.web.packages.select(Packages.find.byId(encrypt.decrypt(id).toLong), bookingForm))
  }

  def book(packageId: String) = UserAwareAction { implicit request =>
    bookingForm.bindFromRequest().fold(
      error => {
        BadRequest(views.html.web.packages.select(Packages.find.byId(encrypt.decrypt(packageId).toLong), error))
      },
      success => {
        val conf = play.Configuration.root()
        val booking = new Bookings
        booking.setUserId(request.identity.orNull)
        ...
        booking.setTransactionRef(TransRefAlgorithm.getUniqueRef(SalesCategory.B2C).getRefCode)
        booking.setSalesCategory(SalesCategory.B2C)
        booking.insert()
        ...
        Ok(views.html.web.packages.thankyou(packageBooking))
      }
    )
  }
