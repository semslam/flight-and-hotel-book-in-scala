...
class AuthCtrl @Inject() (
    val silhouette: Silhouette[MyEnv[PrivateUsers]],
   ...
   */
  def forgotPassword(email: String) = UserAwareAction { implicit request =>
    request.identity match {
      case Some(_) => Redirect(routes.ApplicationCtrl.index)
      case None => Ok(viewsAuth.forgotPassword(emailForm.fill(email)))
    }
  }
      ...
      passwords => {
        tokenService.retrieve(tokenId).flatMap {
          case Some(token) if !token.isExpired => {
            ...
            managerService.retrieve(loginInfo).flatMap {
              case Some(manager) => {
                manager.setPassword(new BCryptPasswordHasher().hash(passwords._1).password)
                manager.setStatus(models.Status.Active)
                manager.update()
                for {
                  _ <- authInfoRepository.update(loginInfo, passwordHasherRegistry.current.hash(passwords._1))
                  authenticator <- env.authenticatorService.create(manager.getEmail)
  ...
  def handleChangePassword = SecuredAction.async { implicit request =>
    changePasswordForm.bindFromRequest.fold(
      ...
      passwords => {
        credentialsProvider.authenticate(Credentials(request.identity.getEmail, passwords._1)).flatMap { loginInfo =>
          for {
